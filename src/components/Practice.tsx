...
import { Target, BookOpen, PenTool, Archive, Sparkles, Loader2, ChevronRight, CheckCircle2, History, Trophy, Star, Award, Zap, Send, HelpCircle, ListOrdered, RefreshCw, Search, Filter, Trash2, Check, Clock, MessageSquare } from 'lucide-react';
import ReactMarkdown from 'react-markdown';
import { generateExample, generateExercise, generateChallenge, generateQuizQuestion, getSocraticResponse } from '@/src/lib/gemini';
                      ...
                      <div className="space-y-4">
                        <div className="relative">
                          <input 
                            ...
                            className="w-full px-8 py-6 rounded-[2.5rem] border border-slate-200 focus:ring-4 focus:ring-brand-500/10 focus:border-brand-500 outline-none transition-all bg-white text-xl shadow-lg pr-20"
                          />
                          <button 
                            onClick={handleSubmitAnswer}
                            disabled={isAnswered || !userAnswer.trim()}
                            className="absolute right-4 top-1/2 -translate-y-1/2 p-4 bg-brand-500 text-white rounded-full hover:bg-brand-600 transition-all disabled:opacity-50"
                          >
                            <Send size={24} />
                          </button>
                        </div>
